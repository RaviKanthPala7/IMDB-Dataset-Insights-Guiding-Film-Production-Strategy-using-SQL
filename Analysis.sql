...
select *,rank() over (order by actor_avg_rating desc,movie_count desc) as actor_rank
from
actor_summary;
...
SELECT *,
       RANK() OVER (ORDER BY actress_avg_rating DESC, movie_count DESC) AS actress_rank
